...
export class RequestsDAL {
  ...
  static async findByUserId(
    ...
    if (!fetchAll) {
      sql += ` AND r.status != 'draft' AND r.status != 'archived'`;
    }

    sql += ` ORDER BY r.created_at DESC`;

    const requests = await Database.query<any>(sql, params);
  ...
  static async create(
    requestData: Omit<AllowanceRequest, "id" | "createdAt" | "updatedAt">
  ): Promise<string> {
  ...
  static async update(
    ...
    try {
      ...
      if (otherUpdates.startDate)
        mappedUpdates.start_date = otherUpdates.startDate;
      ...
      if (otherUpdates.zoneMultiplier)
        mappedUpdates.zone_multiplier = otherUpdates.zoneMultiplier;
      if (otherUpdates.totalAmount)
        mappedUpdates.total_amount = otherUpdates.totalAmount;
      ...
      if (documents !== undefined) {
        ...
        await connection.execute(
          "DELETE FROM request_documents WHERE request_id = ?",
          [id]
        );
        ...
        if (Array.isArray(documents) && documents.length > 0) {
          const docInsertSql =
            "INSERT INTO request_documents (request_id, file_name, file_url, uploaded_at) VALUES ?";
  ...
  static async findByDepartment(department: string): Promise<AllowanceRequest[]> {
    ...
    const requests = await Database.query<any>(sql, [department]);
  ...
  static async findAllWithDetails(): Promise<AllowanceRequest[]> {
    ...
    const requestsWithDetails = await Promise.all(
      requests.map(async (request) => {
        ...
        const approverComment = comments.find(
          (c) => c.message.includes("approved") || c.message.includes("rejected")
        );
